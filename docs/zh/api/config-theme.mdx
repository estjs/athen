...
默认主题配置位于根配置中的 `themeConfig` 下。多语言场景下的 nav/sidebar 放在顶层 `locales` 中。
...
网站的侧边栏。设置为 `'auto'` 时，Athen 会根据 Markdown/MDX 路由元数据自动生成 sidebar：
...
页面可以通过 front matter 从 sidebar 中排除：
