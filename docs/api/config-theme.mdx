...
The default theme config is under `themeConfig` in root config. For locale-specific nav/sidebar, use top-level `locales`.
...
The sidebar of the site. Set it to `'auto'` to generate sidebar groups from Markdown/MDX route metadata:
...
You can also mix auto-generated and manual sections:
  ...
  themeConfig: {
    sidebar: {
      ...
      "/api/": [
        {
          text: "API",
          items: [{ text: "Config", link: "/api/config-basic" }],
        },
      ],
    },
  },
...
Pages can opt out with front matter:
...
Manual sidebar config is still supported:
...
Links displayed at the end of the nav bar, commonly used for GitHub, Discord,
Twitter, and other external destinations.
