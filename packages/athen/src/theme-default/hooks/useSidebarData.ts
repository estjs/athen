...
import type { ResolvedLocaleData, Sidebar, SidebarGroup } from '@shared/types';
...
export function resolveSidebarData(
  ...
  for (const name of Object.keys(sidebarByPath)) {
    ...
    const group = items.find((group) =>
      group.items.some(
        (item) => 'link' in item && item.link && isEqualPath(item.link, decodedPathname),
      ),
    );
    if (group) {
...
export function useSidebarData(
  ...
  sidebar: Computed<ResolvedLocaleData>,
      ...
      Object.entries(sidebar.value.sidebar || {}).map(([name, items]) => [
        ...
        items.map((group) => ({
          ...
          items: group.items.map((item) =>
            'link' in item && item.link ? { ...item, link: withBase(item.link) } : item,
          ),
        })),
      ]),
    );
