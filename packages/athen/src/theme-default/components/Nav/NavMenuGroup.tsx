...
export function NavMenuGroup({ activeIndex, isTranslation, items, text }: NavMenuGroupItem) {
  ...
  return (
    <div
      class="relative b-x-2 b-x-border-default b-x-solid px-12px"
    ...
    >
      <button
        ...
        class="flex items-center gap-4px text-1 text-sm font-medium transition-color duration-200 hover:text-text-2"
      >
        {isTranslation ? <Translator height="18" width="18"></Translator> : text}
        <Down height="18" width="18" />
      </button>
      <div
        class="absolute right-0 top-10 mx-0.8 transition-opacity duration-300"
        style={{
          opacity: isOpen.value ? 1 : 0,
          visibility: isOpen.value ? 'visible' : 'hidden',
        }}
      >
        <div class="z-100 mr-[1.5rem] h-full max-h-100vh min-w-128px w-full overflow-y-auto b-1 b-border-default rounded-xl b-solid bg-bg-default p-3 shadow-[var(--at-shadow-3)]">
                  ...
                  <Link href={child.link}>
                    <div class="rounded-md py-1.6 pl-3 hover:bg-bg-mute">
                      <div class="flex items-center">
                        ...
                        <div class="ml-1 mt-0.5 text-text-3">
